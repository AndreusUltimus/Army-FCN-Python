...
Activations = {
    ...
    'rlinear' : {lambda x: x if x>0 else 0, lambda x: 1 if x>=0 else 0},
    'sin' : (np.sin,np.cos),
    }
...
def get_activation( name ):
    if name=='random':
        return np.random.choice(valid_activations_for_random)
    else:
        return name
...
class Neuron:
    def __init__( self, N_in = 1, activation_type = 'tanh_mon',epsilon=1.):
        self.activation, self.activation_slope = Activations[activation_type]
        self.N_in = N_in
        # rand_func = np.random.random # Uniform [0,1)
        # self.weights = (rand_func(self.N_in)*2. - 1.)*epsilon
        rand_func = np.random.rand # Standard normal
        self.weights = (rand_func(self.N_in))*epsilon
...
class FCNForward( sklearn.base.BaseEstimator ):
    ...
    def fit( self, X, y ):
        ...
        neurons = [
            Neuron(N_in = N_i + 1 + n,activation_type=get_activation(self.hidden_activation),epsilon=self.epsilon)
        ...
        if self.minibatch is None:
            pattern_ranges = [range(N_p)]
        else:
            shuffled = range(N_p)
            np.random.shuffle( shuffled )
            pattern_ranges = [
                shuffled[i_batch*self.minibatch:(i_batch+1)*self.minibatch]
                for i_batch in range(N_p/self.minibatch+1)
                ]
            # print pattern_ranges
            # print shuffled
        ...
        for i_iter in range(self.max_iter):
            for pattern_range in pattern_ranges:
            ...
            for i_p in range(N_p):
                ...
                # E_new += e.dot(e)
                for i_o in range(N_o):
                    E_all += (e_mini[i_o]*cweights[i_p])**2   
                    
            if self.verbose >= 9:
                print "%6i %e errs old %6.1f new %6.1f diff %6.1f" % (i_iter, mu, E_old,E_all,E_old-E_all)
            elif (i_iter%20==0) and (self.verbose>=2):
                print "%6i %e errs old %6.1f new %6.1f diff %6.1f" % (i_iter, mu, E_old,E_all,E_old-E_all)
            ...
            if E_old != -1:
                ...
                if E_old <= self.tol and E_all <= E_old:
                    if (self.verbose>=1):
                        print "Tolerance reached.",
                        print "%6i %e %8f %8f" % (i_iter, mu, E_all, E_old)
                    self.converged_ = True
                    break
                elif abs(E_old-E_all) < self.tol*1e-3:
                    if (self.verbose>=1):
...
class FCNForward_Classify( FCNForward ):
    def __init__(self, adjust_threshold=False, *args, **kwargs ):
        FCNForward.__init__(self,*args,**kwargs)
    ...
    def predict( self, X ):
        ...
        # print y.min(), y.max()
        return (y>self.thresh).astype(np.float)
