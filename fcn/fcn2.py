...
class FCNForward( sklearn.base.BaseEstimator ):
    ...
    def predict(self,X):
        ...
        N_blocks = N_p // n_per_block
        if (N_p<n_per_block) or (N_p % n_per_block):
            N_blocks += 1
            
        Y = np.zeros( [n_per_block,1+N_i+N_n] )
        Y[:n_per_block,0] = 1.        
        
        # for i_p in range(N_p):
        for i_b in range(N_blocks):
            i_start = i_b*n_per_block
            i_end   = min( (i_b+1)*n_per_block, N_p )
            n_this_block = i_end-i_start
            
            Y[:n_this_block,1:1+N_i] = X[i_start:i_end,:]  # Inputs for this pattern
            for i_n in range(N_n):
                netval = np.dot( Y[:n_this_block,:neurons[i_n].N_in], neurons[i_n].weights )
                Y[:n_this_block,1+N_i+i_n] = neurons[i_n].activation( netval )
                # print( i_n, netval, Y[1+N_i+i_n], neurons[i_n].activation)
            y[i_start:i_end,:] = Y[:n_this_block,-N_o:]
            # print( y)
            # print( Y)
    ...
    def fit( self, X, y ):
        ...
        if self.verbose >= 3:
            print( "Training FCN network with %i examples and %i input features..." % X.shape)
            print( "   y: sum()==%f, len()==%i" % (y.sum(), len(y)))
        ...
        for i_iter in range(self.max_iter):
            ...
            for i_b in block_range:
                ...
                for i_n in range(1,N_n):
                    ...
                    delta_w[:n_this_block,:i_n,i_n] = weights[N_i+1:]                
                ...
                for i_n in neuron_range:
                    ...
                    netval = np.dot( Y[:n_this_block,:neuron.N_in],weights )
                    Y[:n_this_block,1+N_i+i_n] = neuron.activation( netval )
                    # print( i_n, netval, Y[1+N_i+i_n])
                    s = neuron.activation_slope( netval )
                    delta_w[:n_this_block,i_n,i_n] = s
                    for j_n in range(i_n):
                        ...
                        #-------------------------------------------------
                        delta_w[:n_this_block,i_n,j_n] = s*np.sum([
                            delta_w[:n_this_block,i_loop,i_n]*delta_w[:n_this_block,i_loop,j_n]
                            for i_loop in range( j_n,i_n )
                            ],axis=0)
                        #-------------------------------------------------
                ...
                i_w = -1
                delta_w_out = delta_w[:n_this_block,-N_o:,:]
                for i_n in neuron_range:
                    for i_in in neurons[i_n].in_range:
                        ...
                        J[:n_this_block,i_w,0] = Y[:n_this_block,i_in]*delta_w_out[:n_this_block,0,i_n]
                        #-------------------------------------------------
                ...
                # TODO: Abuses only one output.
                #-------------------------------------------------
                e_weighted = ( Y[:n_this_block,-1] - y[i_start:i_end,0])*cweights[i_start:i_end]
                #-------------------------------------------------
                # End choice.
                #=================================================            
                ...
                #-------------------------------------------------
                # TODO: Abuses only one output
                E_total += np.sum(np.square( e_weighted ))
                #-------------------------------------------------
                # for i_o in output_range:
            ...
            for m in range(5):
                ...
                # for i_p in pattern_range:
                    ...
                    for i_n in range(N_n):
                        ...
                        if self.dropout:
                            weights = weights * mask[:neurons[i_n].N_in]
                        netval = np.dot( Y[:n_this_block,:neurons[i_n].N_in], weights )
                        
                        Y[:n_this_block,1+N_i+i_n] = neurons[i_n].activation( netval )
                    e_mini = Y[:n_this_block,-N_o:] - y[i_start:i_end,:] 
                    # E_new += e.dot(e)
                    for i_o in range(N_o):
                        E_new += np.sum(np.square(e_mini[:,i_o]*cweights[i_start:i_end]))
            ...
            if ((i_iter%20==0) and (self.verbose>=2)) or (self.verbose>=3):
